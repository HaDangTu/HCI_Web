...
function onConfirmButtonClick(event) {
    ...
    if (Boolean(flag == "true")) {
        ...
        sessionStorage.setItem("transfer-unit", document.getElementById("select-unit").value);
...
function attachData(tableContent, data){
    ...
    for (var i = 1; i < table.rows.length; i++){
        ...
        table.rows[i].cells[3].firstChild.value = data[j].DonVi;
        j += 1;
    }
}
...
function onChooseFileChange() {
    ...
    reader.onload = (progressEvent) => {
        ...
        json_object.forEach((element) => {
            ...
            cell1.innerHTML = '<input type="text" value="' + element.MaKhachHang + '">';
            cellCount++;
            ...
            cell3.innerHTML = '<select id="table-select-data' + cellCount + '" style="width: 150px; font-size: 12px;">' +
                '<option>MoMo</option>' +
                '<option>Viettel Pay</option>' +
                '</select>';
            document.getElementById("table-select-data" + cellCount).value = element.LoaiVi;
            cellCount++;

            //Đơn Vị
            var cell4 = row.insertCell(3);
            cell4.innerHTML = '<select id="table-select-unit' + cellCount + '" style="width: 150px; font-size: 12px;">' +
                '<option>VND</option>' +
                '<option>USD</option>' +
                '<option>YEN</option>' +
                '<option>AUD</option>' +
            ...
            document.getElementById("table-select-unit" + cellCount).value = element.DonVi;
            cellCount++;
        });
    }
...
function getDataFromTable() {
    ...
    for (var i = 1; i < table.rows.length; i++) {
        ...
        var obj = {
            ...
            "DonVi": table.rows[i].cells[3].firstChild.value
        };
...
function onInputChange(rowIndex) {
    if (last_row == rowIndex) {
        ...
        var cellDonVi = row.insertCell(3);
        cellDonVi.innerHTML = '<select style="width: 120px; font-size: 12px;">' +
            '<option>VND</option>' +
            '<option>USD</option>' +
            '<option>YEN</option>' +
            '<option>AUD</option>' +
