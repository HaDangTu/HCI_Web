...
function attachData(tableContent, data){
    ...
    for (var i = 1; i < table.rows.length; i++){
        table.rows[i].cells[0].firstChild.value = data[j].NganHang;
        table.rows[i].cells[1].firstChild.value = data[j].SoTaiKhoan;
        table.rows[i].cells[2].firstChild.value = data[j].Ten;
        table.rows[i].cells[3].firstChild.value = data[j].SoTien;
        table.rows[i].cells[4].firstChild.value = data[j].PhiChuyen;
        table.rows[i].cells[5].firstChild.value = data[j].NoiDung;
        table.rows[i].cells[6].firstChild.value = data[j].DonVi;
        j += 1;
    }
}
function onConfirmButtonClick(event) {
    ...
    if (flag) {  
        ...
        var receiverBankText = document.getElementById("receiver-bank").value;    
        ...
        sessionStorage.setItem("receiver-bank", receiverBankText);
...
function getDataFromTable() {
    ...
    for (var i = 1; i < table.rows.length; i++) {
        ...
        var obj = {
            "NganHang": table.rows[i].cells[0].firstChild.value,
            "SoTaiKhoan": table.rows[i].cells[1].firstChild.value,
            "Ten": table.rows[i].cells[2].firstChild.value,
            "SoTien": table.rows[i].cells[3].firstChild.value,
            "PhiChuyen": table.rows[i].cells[4].firstChild.value,
            "NoiDung": table.rows[i].cells[5].firstChild.value,
            "DonVi" : table.rows[i].cells[6].firstChild.value
        };
        tableData.push(obj);
    }
...
function addRow(rowIndex) {
    ...
    var cellBanking = row.insertCell(0);
    var cellAccountNumber = row.insertCell(1);
    var cellName = row.insertCell(2);
    var cellTransferAmount = row.insertCell(3);
    var cellTransferFee = row.insertCell(4);
    var cellContent = row.insertCell(5);
    var cellUnit = row.insertCell(6);
    
    // cellBanking.className = "table-data";
    cellBanking.innerHTML = '<input type="text" onchange="onInputChange('+ (rowIndex + 1) + ')">';
...
function onChooseFileChange(){
    ...
    reader.onload = (progressEvent) => {
        ...
        json_object.forEach((element) => {
            ...
            cellBanking.innerHTML = '<input type="text" value="' + element.NganHang + '">';

            //SoTaiKhoan
            var cellAccountID = row.insertCell(1);
            // cellAccountID.className = 'table-data';
